...
{
    public static class DigitalSignatureOperation
    {
        ...
        private static Boolean IsPublicKeyInitialized;

        public static void LoadPrivateKey(Byte[] PrivateKey)
        {
        ...
        public static void LoadPublicKey(Byte[] publickey)
        {
            ...
            {
                throw new ArgumentException("Error: Public key can't be null or empty");
            }
            if (!(publickey.Length == 57 || publickey.Length == 32) == true)
            {
                throw new ArgumentException("Error: Public key length must be 57 or 32 bytes in length");
            }
            DateTime ChallengeGenerateDT = Initialization.GetChallengeGenerateDT();
            DateTime CurrentDateTime = DateTime.UtcNow.AddHours(8);
            TimeSpan Duration = CurrentDateTime.Subtract(ChallengeGenerateDT);
        ...
        public static Byte[] Verify(Byte[] SignatureMessage) 
        {
            ...
            if (IsPublicKeyInitialized == true)
            {
            ...
            {
                throw new ArgumentException("Error: Public key of digital signature operation has not been initialized");
            }
